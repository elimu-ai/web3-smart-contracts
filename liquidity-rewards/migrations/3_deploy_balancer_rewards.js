...
module.exports = function (deployer, network, accounts) {
  ...
  if (!isDryRun) {
    ...
    const contractJson = JSON.parse(fs.readFileSync('./build/contracts/' + BalancerPoolRewards.contractName + '.json', 'utf8'));
    console.log('Saving ABI:', './abis/' + network + '/' + BalancerPoolRewards.contractName + '.json.abi');
    fs.writeFileSync('./abis/' + network + '/' + BalancerPoolRewards.contractName + '.json.abi', JSON.stringify(contractJson.abi));
  }
  ...
  if ((network == 'test') || (network == 'soliditycoverage')) {
    deployer.then(async function() {
      ...
      await deployer.deploy(PoolTokenMock, 'Balancer LP Token', 'SLP', deployer.options.from, web3.utils.toWei('1000')); // 1,000
      const poolTokenContract = await PoolTokenMock.deployed();
      console.log('poolTokenContract.address:', poolTokenContract.address);
      await deployer.deploy(BalancerPoolRewards, rewardTokenContract.address, poolTokenContract.address);
    });
    ...
    deployer.deploy(BalancerPoolRewards, rewardTokenAddress, poolTokenAddress);
    ...
    console.log('rewardTokenAddress:', rewardTokenAddress);
    const poolTokenAddress = '0x517390b2B806cb62f20ad340DE6d98B2A8F17F2B';
    console.log('poolTokenAddress:', poolTokenAddress);
    deployer.deploy(BalancerPoolRewards, rewardTokenAddress, poolTokenAddress);
  }
};
