...
contract("BalancerPoolRewards", (accounts) => {
    ...
    it('deployed successfully', async () => {
        this.rewardsContract = await BalancerPoolRewards.deployed()
    ...
    describe('\n💸 Reward Rate', () => {
        it('rewardRatePerSecond() - default value should be 0.249', async () => {
            const rewardRatePerSecond = await this.rewardsContract.rewardRatePerSecond()
            console.log('rewardRatePerSecond:', web3.utils.fromWei(rewardRatePerSecond))
            assert.equal(rewardRatePerSecond, web3.utils.toWei('0.249'))
        })
    ...
    describe('\n💸 Rewards Earned', () => {
        ...
        it('depositPoolTokens() - account1 deposits 10 pool tokens', async () => {
            ...
            // Expect a total pool token supply of 1,000 (see 1_deploy_balancer_rewards.js)
