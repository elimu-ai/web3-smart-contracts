...
contract('SushiswapPoolRewards', function ([_, wallet1, wallet2, wallet3, wallet4, owner]) {
  const deploy = async (that) => {
      that.uni = await ERC20Mock.new('Sushiswap V2', 'UNI-V2', owner, 0);
      that.elimu = await ERC20Mock.new('elimu.ai', 'ELIMU', owner, 0);
      that.pool = await SushiswapPoolRewards.new(that.elimu.address, that.uni.address, { from: owner });
  ...
  describe('SushiswapPoolRewards', async function () {
