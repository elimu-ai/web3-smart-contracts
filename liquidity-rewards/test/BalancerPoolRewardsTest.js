...
contract('BalancerPoolRewards', function ([_, wallet1, wallet2, wallet3, wallet4, owner]) {
  const deploy = async (that) => {
      that.uni = await ERC20Mock.new('Balancer V2', 'UNI-V2', owner, 0);
      that.elimu = await ERC20Mock.new('elimu.ai', 'ELIMU', owner, 0);
      that.pool = await BalancerPoolRewards.new(that.elimu.address, that.uni.address, { from: owner });
  ...
  describe('BalancerPoolRewards', async function () {
