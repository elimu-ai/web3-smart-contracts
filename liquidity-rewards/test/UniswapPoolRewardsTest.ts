...
contract("UniswapPoolRewards", (accounts) => {
    ...
    beforeEach(async () => {
        ...
        if (this.rewardsContract) {
            ...
            console.log(' └── rewardsEarned(address account):')
            for (let i = 1; i < 3; i++) {
                const rewardsEarned = await this.rewardsContract.rewardsEarned(accounts[i])
    ...
    describe('\n💸 Claim Reward - 6 hours after first deposit', () => {
        ...
        it('claimReward() - account1', async () => {
            ...
            const rewardsEarnedAccount2AfterClaiming = await this.rewardsContract.rewardsEarned(accounts[2])
    ...
    describe('\n💸 Withdraw Pool Tokens - 7 hours after first deposit', () => {
        it('withdrawPoolTokens() - account2 withdraws 10 pool tokens', async () => {
            ...
            const totalDepositedByAccount2AfterWithdrawal = await this.rewardsContract.balanceOf(accounts[2])
